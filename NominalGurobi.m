%% Description: Deterministic Problem-G/M/1
clear
clc

% Inputs:
% I: The set of demand points
% J: The set of potential facility locations
% f: Setup cost (J)
% c: Access cost (I*J)
% xinom: Nominal demand (I)
% K: Number of breakpoints
tic
load('p470.mat');

f=10*ones(J,1);
t=100; % Waiting time penalty
CV=2; % CV
R=(CV^2+1)/2;
params.MIPGap = 0.001;
params.TIME_LIMIT = 10000;
% params.NonConvex=2;
%% Piecewise Approximation-Generating Breaking Points
%g=@(q)q^2/(1-q);
%gderiv=@(q)(1/(1-q)^2)-1 ----> First Derivative of g
ghat=@(p)p^2/(1-p);%----> Linear Approximation
%T=@(q)q^2*(1+ghat(p)+p)-2*q*(ghat+p)+ghat(p)----> for finding q [substitute ghat(p)=g(p)-e]
%B=@(p)p^2*(1+gderiv(q))-p*(gderiv(q)+gderiv(q)*q-g(q)-e)+gderiv(q)*q-g(q)-e
p=0;
e=0.001;
bp=0; % Breaking Points
Tp=[]; % The set of Tangency Points
while p < 0.99
    r1=[1/(1-p)-e -2*p/(1-p)+2*e p^2/(1-p)-e];
    qq=roots(r1);
    for i=1:size(qq,1)
        if qq(i) > p
            q=qq(i);
        else
        end
    end
    Tp=[Tp q];
    r2=[1/(1-q)^2 e-2*q/(1-q)^2 q^2/(1-q)^2-e];
    pp=roots(r2);
    for n=1:size(pp,1)
        if pp(n) > q
            pnew=pp(n);
        else
        end
    end
    bp=[bp pnew];
    p=pnew;
end

K=length(bp);
ghatnew=zeros(1,K);
for k=1:K
ghatnew(:,k)=ghat(bp(k));
end

%% Problem
% Order of variables:  y_ij, mu_j, rho_j, theta_j, lambda_jk
% Equality Constraints:
%(3b):
Aeq1=[repmat(speye(I,I),1,J),sparse(I,J), sparse(I,J), sparse(I,J), sparse(I,J*K)];
beq1=ones(I,1);

%(3d):
% bp: Breakpoints (1*K vector)
Aeq2=[sparse(J,I*J), sparse(J,J), speye(J,J), sparse(J,J), -kron(speye(J,J),bp)];
beq2=zeros(J,1);

%(3e):
%B=repmat(reshape(repmat(g,J,1),1,K*J),J,1);
Aeq3=[sparse(J,I*J), sparse(J,J), sparse(J,J), speye(J,J), -kron(speye(J,J),ghatnew)];
beq3=zeros(J,1);

%(3f):
Aeq4=[sparse(J,I*J),sparse(J,J), sparse(J,J), sparse(J,J), kron(speye(J,J),ones(1,K))];
beq4=ones(J,1);

model.A =sparse([Aeq1;Aeq2;Aeq3;Aeq4]);
model.rhs=[beq1;beq2;beq3;beq4];
model.sense=repmat('=',1,I+3*J);

% SOC Constraint:
%(3g):

% Order of variables:  y_ij, mu_j, rho_j, theta_j, lambda_jk
for j=1:J
Q=zeros(I*J+3*J+J*K,I*J+3*J+J*K);
Q((j-1)*I+1:j*I,(j-1)*I+1:j*I)=diag(xinom);
Q(I*J+j,I*J+J+j)=-1; 
model.quadcon(j).Qc= sparse(Q);
model.quadcon(j).q = zeros(I*J+3*J+J*K,1);
model.quadcon(j).rhs = 0;
model.quadcon(j).name = sprintf('cone%d',j);
end

% SOS2
for i = 1:J
    model.sos(i).type = 2;
    model.sos(i).index = I*J+3*J+K*(i-1)+[1:K]';
    model.sos(i).wieght = [1:K]';
end

% Objective function
% For y_ij
D=reshape(c.*repmat(xinom',1,J),1,I*J);
Obj=[D,f',t.*ones(1,J),t*R.*ones(1,J),zeros(1,J*K)];
model.obj=Obj;
model.modelsence='Min';

model.vtype = [repmat('B',1,I*J),repmat('C',1,3*J+J*K)];
model.lb = zeros(1,I*J+3*J+J*K);
% model.lb = [zeros(1,I*J),(-1e-5).*ones(1,J),zeros(1,2*J+J*K)];              % SOC constraints would be treated as non-convex constraints
model.ub = [ones(1,I*J),Inf(1,J),ones(1,J),Inf(1,J),ones(1,J*K)];

gurobi_write(model, 'qcp.lp');
result = gurobi(model, params);

sol = result.x;
val = result.objval;
y=reshape(sol(1:I*J),I,J);
mu=sol(I*J+1:I*J+J);
rho=sol(I*J+J+1:I*J+2*J);
theta=sol(I*J+2*J+1:I*J+2*J+J);
lambda=reshape(sol(I*J+3*J+1:I*J+3*J+J*K),J,K);
accesscost = reshape(c.*repmat(xinom',1,J),1,I*J)*reshape(y,I*J,1);
setupcost = f'*mu;
waitingcost = t*R*sum(theta)+t*sum(rho);
accpercentage = 100*(accesscost/val);
setuppercentage = 100*(setupcost/val);
waitingpercentage = 100*(waitingcost/val);
utilization = 100*rho;
% save NP1b70 y mu rho lambda accpercentage setuppercentage waitingpercentage utilization

timeElapsed = toc;

val
accpercentage
setuppercentage
waitingpercentage
min(utilization)
max(utilization)
timeElapsed
